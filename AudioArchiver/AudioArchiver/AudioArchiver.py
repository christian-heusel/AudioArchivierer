...
class AudioArchiver():
    def __init__(self, check=False):
        ...
        self.srcFile = self.searchForInputFile(
        )  #basePath + "AHQU/USBREC/QU-ST001.WAV"
        #self.srcFileRename =  basePath + "AHQU/USBREC/" +  time.strftime("%Y_%m_%d") + ".WAV"
        self.srcFileRename = basePath + "backup_Aufnahmen/" + time.strftime(
            "%Y_%m_%d") + ".WAV"

        self.destPath = basePath + "Gottesdienst_Archiv/" + time.strftime(
            "%Y/%B/")
        self.destFile = self.destPath + time.strftime(
            "Gottesdienst_%d_%m_%Y") + ".mp3"
        ...
        if check == True:
            if not os.path.isfile(self.srcFile):
                raise NameError("Aufnahme Datei " + self.srcFile +
    ...
    def checkOutputFile(self):
        print(
            "  Überprüfe Ablagedatei um ein überschreiben der Aufnahmen zu verhindern"
        )
        checkFilename = self.basePath + "AHQU/USBREC/" + self.myTime.strftime(
            "%d_%m_%Y") + ".WAV"
...
def main():
    try:
        ...
        parser.add_argument(
            ...
            action="store_true")
        parser.add_argument("--NotCopy",
                            help="not copy the converted mp3 file to sticks",
                            action="store_true")
        ...
        if not args.NotCopy:
            print(audioArchiver.destFile +
                  " auf alle gefunden USB-Stickes kopieren")
